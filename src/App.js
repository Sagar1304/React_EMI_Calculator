...
function App() {
  ...
  const [interest, setIntersest] = useState(10);
  ...
  const calculateDownPayment = (mei) => {
    if (!cost) return;
  ...
  return (
    <div className="App">
      <span className="title" style={{ fontSize: 30, textAlign: "center" }}>
        EMI Calculator
      </span>
      <TextInput
      ...
      <TextInput
        title={"Intrest Rate(in %)"}
        state={interest}
        setState={setIntersest}
      />
